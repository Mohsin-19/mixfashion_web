<section class="content-header">
  ...
  <div class="row">
    <div class="col-md-1">
      <a href="<?php echo base_url() . 'Inventory/inventory' ?>" class="btn btn-block btn-primary pull-left"><strong><?php echo lang('back'); ?></strong></a>
    </div>
              ...
              ?>
                    <tr>
                      ...
                      <?php if ($value->conversion_rate == 0 || $value->conversion_rate == '') { ?>
                        <th class="c_center"><?php echo (getLastPurchaseAmount($value->id)) && getLastPurchaseAmount($value->id) > 0 ? (round(getLastPurchaseAmount($value->id) / 1, 2)) : 'N/A' //getLastPurchaseAmount($value->id) 
                                              ?></td>
                        <?php } else { ?>
                        <th class="c_center"><?php echo (getLastPurchaseAmount($value->id)) && getLastPurchaseAmount($value->id) > 0 ? (round(getLastPurchaseAmount($value->id) / $value->conversion_rate, 2)) : 'N/A' //getLastPurchaseAmount($value->id) 
                                              ?></td>
                        <?php } ?>

                        <th class="c_center"><span style="<?php echo ($totalStock <= $value->alert_quantity) ? 'color:red' : '' ?>"><?php echo escape_output($value->alert_quantity) ?><?php echo " " . $value->sale_unit_name ?></span>
                          </td>
