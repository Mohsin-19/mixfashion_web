...
var cart_not_empty_want_to_clear1 = $('custom_zakir').attr('data-cart_not_empty_want_to_clear');
var add_at_least_one_qty = $('[data-add_at_least_one_qty1]').attr('data-add_at_least_one_qty1');
var warning = $('custom_zakir[data-warning1]').attr('data-warning1');
var ok = $('custom_zakir[data-ok1]').attr('data-ok1');
var no_access = $('custom_zakir[data-no_access]').attr('data-no_access');
var less_stock_msg = $('custom_zakir[data-less_stock_msg]').attr('data-less_stock_msg');
var please_select_an_order = $('custom_zakir[data-please_select_an_order]').attr('data-please_select_an_order');
var please_select_hold_sale = $('custom_zakir[data-please_select_hold_sale]').attr('data-please_select_hold_sale');
var sure_delete_this_hold = $('custom_zakir[data-sure_delete_this_hold]').attr('data-sure_delete_this_hold');
var are_you_delete_all_hold_sale = $('custom_zakir[data-are_you_delete_all_hold_sale]').attr('data-are_you_delete_all_hold_sale');
var sure_delete_this_order = $('custom_zakir[data-sure_delete_this_order]').attr('data-sure_delete_this_order');
var cart_not_empty = $('custom_zakir[data-cart_not_empty]').attr('data-cart_not_empty');
var no_hold = $('custom_zakir[data-no_hold]').attr('data-no_hold');
var select_a_customer = $('custom_zakir[data-select_a_customer]').attr('data-select_a_customer');
var item_modal_status = $('custom_zakir[data-item_modal_status]').attr('data-item_modal_status');
var edit_warning = $('custom_zakir[data-edit_warning]').attr('data-edit_warning');
var a_error = $('custom_zakir[data-a_error]').attr('data-a_error');
var collect_tax = $('[data-collect-tax][data-collect-tax]').attr('data-collect-tax');
...
var print_format = $('base[print_format]').attr('print_format');
var modal_id = [];

var csrf_value_ = $("#csrf_value_").val();

$('.modal-content').each(function(i, obj) {
    modal_id.push($(this).attr('id'));
});
// console.log(modal_id);
$(document).ready(function(){
    /*$.ajax({
            url:base_url+"Sale/check_version_file",
            method:"POST",
            data:{
                version : '1_0.txt',
    ...
    document.onkeyup = function(e) {
        ...
        if (e.ctrlKey && e.altKey && e.which == 67) {
            //press c/C
            var customer_id = $("#walk_in_customer").data('select2');
            customer_id.open();
    ...
    // $(document).not('.modal-content').click(function() {
    //     console.log('arif');
    //     $('.modal').slideUp('500');
    // });
    // $(document).on('click','body',function(e){
    // 	if (is_inside_or_outside_of_modal(e.target.id,e)) {
    //     // console.log(e.target.id);
    //     // if(modal_id.includes(e.target.id) || $(e.target).parents("#"+e.target.id).length){
    //       alert("Inside div");
    //     } else {
    //       alert("Outside div");
    //     }
    ...
    $('#search_barcode').on('keyup',function(e){
        ...
        if(key == 13 && this_value!=""){
            var item_id = find_item_by_item_code(this_value).item_id;
            ready_item_modal(item_id);
            $(this).val('');
        }
    });
        ...
        if(selected_customer!=""){
            if(selected_customer==1){
                ...
                $.ajax({
                    ...
                    async:false,
                    dataType:'json',
                    data:{controller:"19",function:"edit"},
                    success:function(response) {
                        if(response==false){
                            swal({
                                title: warning+"!",
    ...
    $('#kitchen_waiter_bar_button').on('click',function(){
        $('#kitchen_bar_waiter_panel_button_modal').slideDown('500');
    });
    $('#submit_table_modal').on('click',function(){
        $('#show_tables_modal2').slideUp('500');
    });
    ...
    $('#please_read_table_modal_button').on('click',function(){
        $('#please_read_modal').slideDown('500');
    });
    ...
    $(document).on('keyup','#search',function(){
        ...
        if(this_value.length>0){
            ...
            $(".reset_filed").hide();
            if( $(window).width() <=800 ){
                $('.cat-list-item').slideUp(555);
            }
        }

    });
    $(document).on('click','.reset_filed',function(){
        $("#search").val('');
        $(".reset_filed").hide();
        var searched_string = $("#search").val().trim();
        ...
        for (var key in foundItems) {
            if (foundItems.hasOwnProperty(key)) {
                searched_category_items_to_show += '<div  data-last_purchase_price="'+foundItems[key].last_purchase_price+'"  data-whole_sale_price="'+foundItems[key].whole_sale_price+'"  class="single_item fix" id="item_'+foundItems[key].item_id+'">';
                ...
                searched_category_items_to_show += '<img src="'+foundItems[key].image+'" alt="">';
                searched_category_items_to_show += '<p class="item_name">'+foundItems[key].item_name+foundItems[key].brand_name+' ('+foundItems[key].item_code+')</p>';
    ...
    $('.slimScrollDivCategory').slimscroll({
        // height: '562px'
    }).parent().css({
        background: 'none',
        border: '0px solid #184055'
    ...
    $('#calculator_button').on('click',function(e){
        e.preventDefault();
    ...
    $('#notification_close').on('click',function(){
        $('#notification_list_modal').slideUp('500');
        $('.single_notification_checkbox').prop('checked', false);
    ...
    $('#notification_button').on('click',function(){
        ...
        $('#notification_list_modal').slideDown('500');
    });
    ...
    $('#open_hold_sales').on('click',function(){
        $('#show_sale_hold_modal').slideDown('500');
        get_all_hold_sales();
    });
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                for (var key in response.items) {
                    ...
                    var this_unit_name = this_item.unit_name;
                    if(this_unit_name==null || this_unit_name==''){
                        this_unit_name = "Pcs";
                    }
                    ...
                    draw_table_for_hold_order += '<div class="single_order_column_hold third_column column fix"><span id="hold_item_quantity_table_'+this_item.food_menu_id+'">'+this_item.qty+'</span> <span class="cart_item_unit_name">'+this_unit_name+'</span></div>';
                    ...
                    if(this_item.menu_note!=""){
                        ...
                        draw_table_for_hold_order += '<div class="single_order_column_hold first_column column fix">IMEI/Serial: <span id="hold_item_note_table_'+this_item.food_menu_id+'">'+this_item.menu_note+'</span></div>';
                        draw_table_for_hold_order += '</div>';
                    }
                ...
                $('#total_items_in_cart_hold').html(response.total_items);
            ...
            success:function(response) {
                ...
                for (var key in response.items) {
                    ...
                    if(this_item.menu_note!=""){
                        ...
                        draw_table_for_last_ten_sales_order += '<div class="single_order_column_hold first_column column fix">IMEI/Serial: <span id="last_10_item_note_table_'+this_item.food_menu_id+'">'+this_item.menu_note+'</span></div>';
                        draw_table_for_last_ten_sales_order += '</div>';
                    }
                ...
                $('#total_items_in_cart_last_10').html(response.total_items);
                ...
                var total_vat_section_to_show = '';
                $.each(JSON.parse(response.sale_vat_objects), function(key, value){
                    total_vat_section_to_show += '<span class="tax_field_order_details" style="padding-right: 5%;" id="tax_field_order_details_'+value.tax_field_id+'">'+value.tax_field_type+'</span>  '+currency+' <span style="padding-right: 4px;" class="tax_field_amount_order_details" id="tax_field_amount_order_details_'+value.tax_field_id+'">'+value.tax_field_amount+'</span><br/>';
                });
                ...
                $("#paid_amount_last_10").html(response.paid_amount);
                $("#due_amount_last_10").html(response.due_amount);
    ...
    $('#delete_all_hold_sales_button').on('click',function(){
        if($('.detail_hold_sale_holder .single_hold_sale').length>0){
            $.ajax({
                ...
                async:false,
                dataType:'json',
                data:{controller:"4",function:"delete"},
                success:function(response) {
                    if(response==false){
                        swal({
                            title: warning+"!",
                        ...
                        if (r == true) {
                            ...
                            $.ajax({
                                ...
                                success:function(response) {
                                    ...
                                    $('#show_sale_hold_modal').slideUp('500');
                                },
                                error:function(){
                                    alert(a_error);
                                }
    ...
    $('#create_invoice_and_close,#order_details_create_invoice_close_order_button').on('click',function(){
        ...
        if($('.holder .order_details > .single_order[data-selected=selected]').length>0){
            ...
            $.ajax({
                ...
                success:function(response) {
                    response = JSON.parse(response);
                    $('#finalize_grand_total').html(response.total_payable);
    ...
    $('#pay_amount_invoice_input').on('keyup',function(e){
        ...
        if(Number(paid_amount)==0){           
            $("#finalie_order_payment_method").prop("disabled", true);          
        }else{            
            $("#finalie_order_payment_method").prop("disabled", false);
        }
        //Added by Alauddin 
    ...
    $('.main_right').css('height',height_should_be+'px');
    //end
    ...
    $('#modal_item_price_input_field').on('keyup',function(){
        update_all_total_price();
    });
    $('#item_quantity_modal_input').on('keyup',function(){
        update_all_total_price();
    });
    $(document).on('click','.increase_item_qty',function(){
        var item_id = $(this).attr('data-item_id');
        //for check qty

        var item_object = find_item_by_item_id(item_id);
        if(item_object.item_type==1){
            checkQty(item_id);
        }

        $("#item_modal").hide();
        $(".edit_item_increment_"+item_id).click();
        $("#item_modal").hide();
        var qty_default = $('#order_for_item_'+item_id).attr('data-qty_default');
        var current_qty = $("#item_quantity_table_"+item_id).text();
        $("#item_quantity_modal_input").val(Number(current_qty)+Number(qty_default));
        $("#item_quantity_modal").val(Number(current_qty)+Number(qty_default));
        update_all_total_price();
        $("#add_to_cart").click();
    });
    $(document).on('click','.decrease_item_qty',function(){
        var item_id = $(this).attr('data-item_id');
        $("#item_modal").hide();
        $(".edit_item_increment_"+item_id).click();
        $("#item_modal").hide();
    ...
    function checkQty(item_id,item_quantity=''){

        if(qty_setting_check=="Yes"){
            var curr_qty = $('#item_quantity_table_'+item_id).text();
            if(item_quantity!=''){
                curr_qty = (item_quantity-1);
            }
            if((Number(curr_qty)+1)){
                $.ajax({
                    url:base_url+"Ajax/checkQty",
                    method:"post",
                    dataType:'json',
                    async: false,
                    data:{
            ...
            var alert_check = $("#alert_check").val();
            if(alert_check==''){
                swal({
                    title: warning+"!",
                    text: qty_alert,
    ...
    $(document).on('click','.model_price_three',function(){
        var value = $(this).find(".s_sale").html()
        $('#modal_item_price').html(value);
    ...
    $(document).on('click','.single_item',function(){
        ...
        var whole_sale_price = $(this).attr('data-whole_sale_price');
        var last_purchase_price = $(this).attr('data-last_purchase_price');
		

        if(whole_sale_price!='' && whole_sale_price!=null){
            $("#w_s_price").html(parseFloat(whole_sale_price).toFixed(2));
        }else{
            $("#w_s_price").html("0.00");
        }

        if(last_purchase_price!='' && last_purchase_price!=null){
            $("#m_p_price").html(parseFloat(last_purchase_price).toFixed(2));
			$("#modal_item_last_purchase_price_input_field").val(parseFloat(last_purchase_price).toFixed(2)); //added by alauddin
        }else{
            $("#m_p_price").html("0.00");
			$("#modal_item_last_purchase_price_input_field").val("0.00");  //added by alauddin
        }

        var row_number = $('#modal_item_row').html();

        //get item id from right item
        var item_id = $(this).attr('id').substr(5);
        var that_is = $(this);
        var tmp= 1;

        $.when(firstCall()).done(function() {
            //set item id to modal
            $('#modal_item_id').html(item_id);
            if($('#order_for_item_'+item_id).length>0){
                if(item_modal_status=="Show"){
                    $("#item_modal").slideDown();
                }else{
                }
                $(".edit_item_increment_"+item_id).click();
                var qty_default = $('#order_for_item_'+item_id).attr('data-qty_default');
            ...
            var item_object = find_item_by_item_id(item_id);
            $("#modal_item_g_w").val((item_object.warranty?"Warranty: "+item_object.warranty:'')+(item_object.warranty && item_object.guarantee?', ':'')+(item_object.guarantee?"Guarantee: "+item_object.guarantee:''));
            $(".current_stock_t").html(item_object.current_stock);
            //set default qty/amt if exist
            var default_qty_amt = (item_object.default_qty_amt=="" || item_object.default_qty_amt==null || item_object.default_qty_amt==0)?1: item_object.default_qty_amt;
            $('#item_quantity_modal_input').val(default_qty_amt);

            //set item sale unit
            if(item_object.item_type==2){
                $('#sale_unit_name_modal').html("Pcs");
            }else{
                $('#sale_unit_name_modal').html(item_object.sale_unit_name);
            }
            //get item/menu name from right item
            var item_name = that_is.find('.item_name').html();
            //set item/menu name to modal
            $('#modal_item_name').html(item_name);
            $('#modal_item_type').html(item_object.item_type);

            //discount amount from modal
            var item_discount_amount = parseFloat(0).toFixed(2);
            //set discount amout to modal

            if(Number(item_discount_amount)){
                $('#modal_discount').val(item_discount_amount);
            }else{
                $('#modal_discount').val('');
            }
            ...
            //get item/menu price from right side
            var item_price = (parseFloat(that_is.find('.item_price').html().substr(2))).toFixed(2);
            //set item/menu price to modal
            $('#modal_item_price').html(item_price);
            $('#s_price').html(item_price);
            $('#modal_item_price_input_field').val(item_price);
            $('#modal_item_price_variable').html(item_price);
            $('#modal_item_price_variable_without_discount').html(item_price);
            ...
            draw_table_for_order += (row_number>0)?'':'<div class="single_order fix" data-qty_default="'+default_qty_amt+'" id="order_for_item_'+item_id+'">';
		...
		//	draw_table_for_order += '<span class="item_type" id="last_purchase_price_table'+item_id+'" style="display:none;"></span>'; //added by alauddin
            ...
            draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox access_control" id="percentage_table_'+item_id+'" value="'+discount_input_value+'"></div>';
            ...
            if(note!=""){
                ...
                draw_table_for_order += '<div class="single_order_column first_column fix">IMEI/Serial: <span id="item_note_table_'+item_id+'">'+note+'</span></div>';
                draw_table_for_order += '</div>';
            }
    ...
    $('#cancel_button').on('click',function(){
        ...
        if(total_items_in_cart>0){
            var x = confirm(cart_not_empty_want_to_clear1);
            if(x == true){
    ...
    $(document).on('click','.edit_item',function(event){
        ...
        var cart_item_unit_name = single_order_element_object.find('.cart_item_unit_name_'+menu_id).html();
        ...
        $('#modal_item_name').html(item_name);
        $('#sale_unit_name_modal').html(cart_item_unit_name);
        $('#modal_item_price').html(item_price);
        $('#s_price').html(item_price);
        $('#modal_item_price_input_field').val(item_price);
        ...
        if(Number(item_discount_input_value)){
        ...
        var click_status = $(this).attr('data-status');
        if (event.which === 1) {
            $('#item_modal').slideDown('500');
        }

        $('.model_price_three').each(function(i,obj){
            var value = $(this).find(".s_sale").html()
            var modal_item_price_input_field = $("#modal_item_price_input_field").val();
    ...
    $('#plus_button').on('click',function(){
        $.ajax({
            ...
            async:false,
            dataType:'json',
            data:{controller:"19",function:"create"},
            success:function(response) {
                if(response==false){
                    swal({
                        title: warning+"!",
                    ...
                    $('#add_or_edit_text').html('Add');
                    $('#add_customer_modal').slideDown('500');
                }
            },
            error:function(){
    ...
    $('#table_button,#dine_in_button').on('click',function(){
        ...
        $('#show_tables_modal2').slideDown('500');
        $('.bottom_person').val('1');
        $.ajax({
    ...
    $('#order_details_close_button').on('click',function(){
        $('#order_detail_modal').slideUp('500');
    });
    ...
    $('#last_ten_sales_close_button,#last_ten_sales_close_button_cross').on('click',function(){
        $('#show_last_ten_sales_modal').slideUp('500');
        reset_last_10_sales_modal();
    });
    $('#keyboard_shortcut_close_button_cross').on('click',function(){
        $('#show_keyboard_shortcut_modal').slideUp('500');
        reset_last_10_sales_modal();
    });
    ...
    $('#keyboard_shortcuts_button').on('click',function(){
        $('#show_keyboard_shortcut_modal').slideDown('500');
    });
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                for (var key in orders) {
                    ...
                    last_10_orders += '<div class="third_column column fix">'+orders[key].sale_date+' '+orders[key].order_time+'</div>';
                    last_10_orders += '</div>';
                }
    ...
    $('#last_ten_sales_button').on('click',function(){
        $('#show_last_ten_sales_modal').slideDown('500');
        getLastSale('','','','default');
    });
    $('.search_sale').on('click',function(){
      var date_c = $("#date_c").val();
      var customer_c = $("#customer_c").val();
      var invoice_c = $("#invoice_c").val();
    ...
    $('#add_to_cart').on('click',function(){
        ...
        //get item_type id from modal
        var item_type = $('#modal_item_type').html();

        if(item_type!=2){

            var current_stock = $(".current_stock_t").html();
            var added_qty = $("#item_quantity_modal_input").val();
            var numb = current_stock.match(/\d/g);
            var current_stock_n = Number(numb.join("")) - Number(added_qty);
            if(current_stock_n<0){
                swal({
                    title: warning+"!",
                    text: less_stock_msg,
                    confirmButtonText:ok,
        ...
        var item_price = (parseFloat($('#modal_item_price_input_field').val())).toFixed(2);
		var item_last_purchase_price = (parseFloat($('#modal_item_last_purchase_price_input_field').val())).toFixed(2); //added by alauddin
		
        if(item_price=='' || item_price== "NaN"){
            item_price = 0;
        }

        //get item/menu sale unit name
        var sale_unit_name = $('#sale_unit_name_modal').html();
        //discount amount from modal
        var item_discount_amount = (parseFloat($('#modal_discount_amount').html())).toFixed(2);
        //discount input value of modal
        var discount_input_value1 = $('#modal_discount').val();

        var discount_input_value = '';
        if(discount_input_value1){
            discount_input_value = discount_input_value1;
        }
        //get item/menu price from modal without discount
        ...
        if(item_quantity=='' || item_quantity=="0.00" || item_quantity==0 || isNaN(item_quantity)){
            add_at_least_one_qty
            swal({
                title: warning+"!",
                text: add_at_least_one_qty,
                confirmButtonText:ok,
                confirmButtonColor: '#b6d6f6'
            });
            return false
        ...
        if(item_type==1){
            checkQty(item_id,item_quantity);
        }
        ...
        tax_information.sort(function (a, b) {
            return a.tax_field_name.localeCompare(b.tax_field_name);
        });
        ...
        var default_qty_amt = $('#item_quantity_modal_input').val();
        //construct div
        var draw_table_for_order='';
        draw_table_for_order += (row_number>0)?'':'<div class="single_order fix" data-qty_default="'+default_qty_amt+'" data-item_type="'+item_type+'" id="order_for_item_'+item_id+'">';
        draw_table_for_order += '<div class="first_portion fix">';
        draw_table_for_order += '<span class="item_previous_id" id="item_previous_id_table'+item_id+'" style="display:none;"></span>';
        draw_table_for_order += '<span class="item_cooking_done_time" id="item_cooking_done_time_table'+item_id+'" style="display:none;"></span>';
        draw_table_for_order += '<span class="item_cooking_start_time" id="item_cooking_start_time_table'+item_id+'" style="display:none;"></span>';
        draw_table_for_order += '<span class="item_cooking_status" id="item_cooking_status_table'+item_id+'" style="display:none;"></span>';
        draw_table_for_order += '<span class="item_type" id="item_type_table'+item_id+'" style="display:none;"></span>';
	//	draw_table_for_order += '<span class="item_type" id="item_last_purchase_price_table'+item_last_purchase_price+'" style="display:none;"></span>'; //added by alauddin

        draw_table_for_order += '<span class="item_vat" id="item_vat_percentage_table'+item_id+'" style="display:none;">'+JSON.stringify(tax_information)+'</span>';

        draw_table_for_order += '<span class="item_discount" id="item_discount_table'+item_id+'" style="display:none;">'+item_discount_amount+'</span>';
        draw_table_for_order += '<span class="item_price_without_discount" id="item_price_without_discount_'+item_id+'" style="display:none;">'+item_total_price_without_discount+'</span>';
        draw_table_for_order += '<div class="single_order_column first_column fix"><i style="cursor:pointer;" data-status="edit" class="fas fa-pencil-alt edit_item_first_call edit_item_increment edit_item_increment_'+item_id+' edit_item" id="edit_item_'+item_id+'"></i> <span class="edit_item_ edit_item_new_'+item_id+'" data-id="'+item_id+'" id="item_name_table_'+item_id+'">'+item_name+'</span></div>';
        draw_table_for_order += '<div class="single_order_column second_column fix"><span class="item_type" id="item_last_purchase_price_table_'+item_id+'" style="display:none;">'+item_last_purchase_price+'</span>'+currency+' <span id="item_price_table_'+item_id+'">'+item_price+'</span></div>';
        draw_table_for_order += '<div class="single_order_column third_column fix"><i style="cursor:pointer;" class="fas fa-minus-circle decrease_item_qty dd" id="dd"  data-item_id="'+item_id+'"></i> <span id="item_quantity_table_'+item_id+'">'+(parseFloat(item_quantity).toFixed(2))+'</span> <i style="cursor:pointer;" class="fas fa-plus-circle increase_item_qty ii" data-item_id="'+item_id+'"></i> <span class="cart_item_unit_name_'+item_id+' cart_item_unit_name">'+sale_unit_name+'</span></div>';
        draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox access_control" id="percentage_table_'+item_id+'" autocomplete="off" onfocus="select();" value="'+discount_input_value+'"></div>';
        draw_table_for_order += '<div class="single_order_column fifth_column fix">'+currency+' <span data-item_type="'+item_type+'" data-total_amount="'+item_total_price+'" id="item_total_price_table_'+item_id+'">'+item_total_price+'</span> <i data-remove-order-row-no="0" class="remove_this_item_from_cart fas fa-times-circle"></i></div>';
        draw_table_for_order += '</div>';
        if (selected_modifiers != "") {
            draw_table_for_order += '<div class="second_portion fix">';
            draw_table_for_order += '<span id="item_modifiers_id_table_' + item_id + '" style="display:none;">' + selected_modifiers_id + '</span>';
        ...
        var modal_item_g_w = $('#modal_item_g_w').val();
        ...
        if(modal_item_g_w!=""){
            draw_table_for_order += '<div class="third_portion fix">';
            draw_table_for_order += '<div class="single_order_column first_column fix" id="item_g_w_table_'+item_id+'">'+modal_item_g_w+'</div>';
            draw_table_for_order += '</div>';
        }
        ...
        //do calculation on table
        do_addition_of_item_and_modifiers_price();
        $("#all_items_options").val('').trigger("change");
    ...
    function checkAccessc() {
        $.ajax({
            ...
            async:false,
            dataType:'json',
            data:{controller:"4",function:"give_discount"},
            success:function(response) {
                if(response==false){
                    $(".access_control").attr('readonly','readonly');
                    $("#sub_total_discount").attr('readonly','readonly');
                    $("#modal_discount").attr('readonly','readonly');
                }
            },
            error:function(){
    ...
    $('#place_order_operation').on('click',function(){
        ...
        var sale_date1 = $('#sale_date1').val();
        var sale_remark1 = $('#sale_remark1').val();



        var total_item_discount_amount = parseFloat($('#total_item_discount').html()).toFixed(2);
        var finalize_grand_total = parseFloat($('#finalize_grand_total').html()).toFixed(2);
        ...
        var select_employee_id = $('#select_employee_id').val();
        var waiter_id = $('#select_waiter').val();
        var sale_vat_objects = [];

        var previous_due = $('#walk_in_customer').find(':selected').attr('data-previous_due');
        $("#finalize_previous_due").html(parseFloat(previous_due).toFixed(2));
        ...
        order_info += '"select_employee_id":"'+select_employee_id+'",';
        order_info += '"total_items_in_cart":"'+total_items_in_cart+'",';
        order_info += '"sub_total":"'+sub_total+'",';
        order_info += '"total_vat":"'+total_vat+'",';
        order_info += '"total_payable":"'+(Number(total_payable)+Number(previous_due))+'",';
        ...
        order_info += '"sale_date1":"'+sale_date1+'",';
        order_info += '"sale_remark1":"'+sale_remark1+'",';
        ...
        if($('.order_holder .single_order').length>0){
            ...
            $('.order_holder .single_order').each(function(i, obj) {
                var item_type_check = $(this).attr('data-item_type');
                var item_id = $(this).attr('id').substr(15);
                var item_name = $(this).find('#item_name_table_'+item_id).html();
				var item_last_purchase_price = $(this).find('#item_last_purchase_price_table_'+item_id).html(); //added by alauddin
                ...
                items_info += '{"item_id":"'+item_id+'", "item_name":"'+item_name+'","item_last_purchase_price":"'+item_last_purchase_price+'", "item_vat":'+item_vat+',';
                ...
                items_info += '"item_cooking_start_time":"'+item_cooking_start_time+'","item_cooking_status":"'+item_cooking_status+'",';
        ...
        var order_object = JSON.stringify(order_info);
        add_sale_by_ajax(order_object,sale_id,total_payable);
    ...
    function pullNewItemInfoWithStock() {
        console.log('reloading items');
        $.ajax({
            url:base_url+"Sale/getItemsWithStock",
            method:"POST",
            dataType: 'json',
            data:{
                csrf_offpos: csrf_value_
    ...
    $('#finalize_order_button').on('click',function(){
        ...
        var send_invoice_sms = $('input:checkbox[name=send_invoice_sms]:checked').val();
        var send_invoice_email = $('#send_invoice_email').val();
        var send_invoice_sms = $('#send_invoice_sms').val();
        var customer_id = $('#walk_in_customer').val();
        var select_employee_id = $('#select_employee_id').val();
        var due_amount_invoice_input = $('#due_amount_invoice_input').val();
        var note = $('#pay_amount_note_invoice_modal_input').val();
        var given_amount_input = $('#given_amount_input').val();
       
        if(customer_id==1 && due_amount_invoice_input>0){
            swal({
                title: warning+"!",
                text: 'Due is not acceptable for Walk-in Customer',
                confirmButtonText:ok,
                confirmButtonColor: '#b6d6f6'
            });
            return false;
        ...
        if(note.length>75){
            swal({
                title: warning+"!",
                text: 'The Note field cannot exceed 75 characters in length.',
                confirmButtonText:ok,
                confirmButtonColor: '#b6d6f6'
            });
            return false;
        ...
        function  addSaleFirst() {
            ...
            var order_object = $("#order_object").val();
                ...
                success:function(response) {
                    ...
                    if(response>0){
                        ...
                        $.ajax({
                            ...
                            success:function(response) {
                                response = JSON.parse(response);
                                $('#modal_finalize_sale_id').html(response.id);
                                $('#finalize_update_type').html('2');//when 2 update payment method, close time and order_status to 3
                                calculate_create_invoice_modal();

                                var sale_id = $('#modal_finalize_sale_id').html();
                                var payment_method_type = $('#finalie_order_payment_method').val();
                                var paid_amount = $('#pay_amount_invoice_input').val();
                                var account_note = $('#pay_amount_note_invoice_modal_input').val(); //added by alauddin
                                
                                var due_amount = $('#due_amount_invoice_input').val();
                                var send_invoice_sms = $('#send_invoice_sms').val();
                                var send_invoice_email = $('#send_invoice_email').val();
                                var invoice_create_type = $('#finalize_update_type').html();
                                var finalize_previous_due = $('#finalize_previous_due').html();
                            //   print_invoice_and_close(sale_id,payment_method_type,invoice_create_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due);
                                print_invoice_and_close(sale_id,payment_method_type,invoice_create_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due,account_note); //added by alauddin
                                pullNewItemInfoWithStock();
                                reset_finalize_modal();
                            },
                            error:function(){
                                alert(a_error+"111");
    ...
    $.ajax({
        ...
        async:false,
        dataType:'json',
        data:{controller:"4",function:"change_price"},
        success:function(response) {
            if(response==false){
                $("#modal_item_price_input_field").attr('readonly','readonly');
            }
        },
        error:function(){
    ...
    $('#add_item_to_right').on('click',function(){
        ...
        $.ajax({
            ...
            async:false,
            dataType:'json',
            data:{controller:"17",function:"create"},
            success:function(response) {
                if(response==false){
                    swal({
                        title: warning+"!",
                    ...
                    $.ajax({
                        url:base_url+"Sale/getItemCode",
                        method:"GET",
                        async:false,
                        dataType:'json',
                        success:function(response) {
                            $("#item_code_modal").val(response);
                        },
                        error:function(){

                        }
                    });
                    $('#add_item_modal').slideDown('500');
                }
            },
            error:function(){
    ...
    $('#add_post_setting').on('click',function(){
        var sms_setting_check = $("#sms_setting_check:checked").val();
        if(sms_setting_check==undefined){
            sms_setting_check = '';
        }
        var qty_setting_check = $("#qty_setting_check:checked").val();
        if(qty_setting_check==undefined){
            qty_setting_check = '';
        }
        $.ajax({
            url:base_url+"Ajax/putSetting",
            method:"GET",
            dataType:'json',
            data:{sms_setting_check:sms_setting_check,qty_setting_check:qty_setting_check},
            success:function(response) {
    ...
    $('#set_qty_alert_sms_setting').on('click',function(){
        $('#show_qty_sms_setting_modal').slideDown('500');
    });
    $('#cancel_set_qty_alert_sms_setting').on('click',function(){
        $('#show_qty_sms_setting_modal').slideUp('500');
    });

    $('#close_add_item_modal').on('click',function(){
        $('#add_item_modal').slideUp('500');
        $('#item_name_modal').val('');
        $('#item_type_modal').val(1).change();
        $('#item_category_modal').val('').change();
        $('#item_brand_modal').val('').change();
        $('#item_sale_unit_modal').val('').change();
        $('#item_sale_price_modal').val('');
        $('#item_purchase_unit_modal').val('').change();
        $('#item_conversion_rate_modal').val('');
        $('#item_purchase_price_modal').val('');
        $('#item_default_qty_amt_modal').val('');
        $('#item_opening_stock_modal').val('');
        $('#item_alert_quantity_modal').val('');
        $('#item_description_modal').val('');
        $('#item_warranty_modal').val('');
        $('#item_guarantee_modal').val('');
        $('.set_zero_tax').val('0');
    });
    $('#add_customer').on('click',function(){
        ...
        var nid = $('#customer_nid_modal').val();
        var customer_email = $('#customer_email_modal').val();
        var customer_group_id = $('#customer_group_id_modal').val();
        var customer_dob = $('#customer_dob_modal').val();
        var customer_doa = $('#customer_doa_modal').val();
        var customer_previous_due_modal = $('#customer_previous_due_modal').val();
        ...
        $.ajax({
            ...
            data:{
                ...
                previous_due: customer_previous_due_modal,
                    ...
                    $.ajax({
                        ...
                        success:function(response) {
                            ...
                            for (var key in response) {
                              var previous_due = 0;
                              if(response[key].previous_due==null){
                                  previous_due = 0;
                              }else{
                                  previous_due = response[key].previous_due;
                              }
                                if(i==response.length){
                                    ...
                                    var tmp_due = 0;
                                    if(customer_previous_due_modal==''){
                                        tmp_due = 0;
                                    }else{
                                        tmp_due = customer_previous_due_modal;
                                    }
                                    $option_customers += '<option id="cid_'+response[key].id+'" data-previous_due="'+tmp_due+'"   value="'+response[key].id+'" selected>'+response[key].name+' '+response[key].phone+'</option>';
                                    ...
                                    $option_customers += '<option  id="cid_'+response[key].id+'" data-previous_due="'+previous_due+'"  value="'+response[key].id+'">'+response[key].name+' '+response[key].phone+'</option>';
                                }
                                i++;
                            }
    ...
    $('#all_items_options').on('change',function(){
        var item_id = $(this).val();
        if($('#order_for_item_'+item_id).length>0){
            swal({
                title: warning+"!",
                text: 'This item already added in cart, you can change Qty/Amt of this item in cart',
                confirmButtonText:ok,
                confirmButtonColor: '#b6d6f6'
            });
            $("#all_items_options").val('').trigger("change");
    ...
    $('#add_item').on('click',function(){
        ...
        var item_type = $('#item_type_modal').val();
        var item_category = $('#item_category_modal').val();
        var item_brand = $('#item_brand_modal').val();
        var item_code_modal = $('#item_code_modal').val();
        var item_sale_unit = $('#item_sale_unit_modal').val();
        var item_sale_price = $('#item_sale_price_modal').val();
        var whole_sale_price = $('#item_whole_sale_price_modal').val();

        var item_purchase_unit_modal = $('#item_purchase_unit_modal').val();
        var item_conversion_rate_modal = $('#item_conversion_rate_modal').val();
        var item_purchase_price_modal = $('#item_purchase_price_modal').val();
        var item_default_qty_amt_modal = $('#item_default_qty_amt_modal').val();
        var item_opening_stock_modal = $('#item_opening_stock_modal').val();
        var item_alert_quantity_modal = $('#item_alert_quantity_modal').val();
        var item_warranty_modal = $('#item_warranty_modal').val();
        var item_guarantee_modal = $('#item_guarantee_modal').val();
        var item_description_modal = $('#item_description_modal').val();
        var supplier_id_modal = $('#supplier_id_modal').val();

        var tax_type_length = $('.this_item_tax_id').length;
        var item_tax_objects = [];
        var error = 0;

        var no_error_bg = '1px solid #B5D6F6';
        var error_bg = '1px solid red';

        $('#item_name_modal').css('border',no_error_bg);
        $('#item_type_modal').next().find('.select2-selection--single').css('border',no_error_bg);
        $('#item_category_modal').next().find('.select2-selection--single').css('border',no_error_bg);
        $('#item_brand_modal').next().find('.select2-selection--single').css('border',no_error_bg);
        $('#item_brand_modal').next().find('.select2-selection--single').css('border',no_error_bg);
        $('#item_sale_unit_modal').next().find('.select2-selection--single').css('border',no_error_bg);
        $('#item_sale_price_modal').css('border',no_error_bg);

        if(item_name==""){
            $('#item_name_modal').css('border',error_bg);
            error++;
            $('.loader').slideUp('500');
        }

        if(item_type==""){
            $('#item_type_modal').next().toggleClass("error_msg", $('#item_type_modal :selected').val() == "");
            error++;
            $('.loader').slideUp('500');
        }

        if(item_sale_price==""){
            $('#item_sale_price_modal').css('border',error_bg);
            error++;
            $('.loader').slideUp('500');
        }
        ...
        $('.this_item_tax_id').each(function(i, obj) {
            var tax_object = {};
            tax_object.tax_field_id = $('.this_item_tax_id').eq(i).val();
            tax_object.tax_field_outlet_id = $('.this_item_tax_outlet_id').eq(i).val();
            tax_object.tax_field_company_id = $('.this_item_tax_company_id').eq(i).val();
            tax_object.tax_field_name = $('.tax_field_name').eq(i).val();
            tax_object.tax_field_percentage = $('.tax_field_percentage').eq(i).val();

            item_tax_objects.push( tax_object );
        });
        ...
        $.ajax({
            url:base_url+"Sale/add_item_by_ajax",
            method:"POST",
            data:{
                item_name: item_name,
                item_type: item_type,
                ...
                item_sale_unit: item_sale_unit,
                item_sale_price: item_sale_price,
                item_tax_objects : item_tax_objects,
                item_purchase_unit_modal : item_purchase_unit_modal,
                item_conversion_rate_modal : item_conversion_rate_modal,
                item_purchase_price_modal : item_purchase_price_modal,
                item_default_qty_amt_modal : item_default_qty_amt_modal,
                item_opening_stock_modal : item_opening_stock_modal,
                item_alert_quantity_modal : item_alert_quantity_modal,
                item_description_modal : item_description_modal,
                item_warranty_modal : item_warranty_modal,
                item_guarantee_modal : item_guarantee_modal,
                supplier_id_modal : supplier_id_modal,
            ...
            success:function(response) {
                ...
                var item_object = JSON.parse(response);
                var purchase_price_temp_1 = 0;
                var whole_sale_price_temp_1 = 0;

                if(item_object.purchase_price == null){
                    purchase_price_temp_1 = 0;
                }else{
                    purchase_price_temp_1 = item_object.purchase_price
                }
                if(item_object.whole_sale_price == null){
                    whole_sale_price_temp_1 = 0;
                }else{
                    whole_sale_price_temp_1 = item_object.whole_sale_price
                }

                var shaped_item_object = {};
                shaped_item_object.category_name = item_object.item_category_name;
                shaped_item_object.brand_name = item_object.item_brand_name;
                shaped_item_object.image = (item_object.photo==null || item_object.photo=='')?base_url+'assets/images/image_thumb.png':base_url+'images/'+item_object.photo;
                shaped_item_object.item_code = item_object.code;
                shaped_item_object.item_id = item_object.id;
                shaped_item_object.item_name = item_object.item_name;
                shaped_item_object.price = currency+' '+item_object.sale_price;
                shaped_item_object.sale_unit_name = item_object.sale_unit_name;
                shaped_item_object.sold_for = item_object.item_sold;
                shaped_item_object.tax_information = item_object.tax_information;
                shaped_item_object.whole_sale_price = whole_sale_price_temp_1;
                shaped_item_object.last_purchase_price = purchase_price_temp_1;
                shaped_item_object.warranty = item_object.warranty;
                shaped_item_object.guarantee = item_object.guarantee;
                shaped_item_object.current_stock = item_object.item_opening_stock_modal;
                window.items.push(shaped_item_object);
                var item_to_show = '<div  data-last_purchase_price="'+purchase_price_temp_1+'"   data-whole_sale_price="'+whole_sale_price_temp_1+'"   class="single_item fix" id="item_'+item_object.id+'">';
                item_to_show += '<p class="item_price">'+currency+' '+item_object.sale_price+'</p>';
                item_to_show += '<img src="'+base_url+'assets/images/image_thumb.png" alt="">';
                if(item_object.item_brand_name){
                    item_to_show += '<p class="item_name">'+item_object.item_name+"-"+item_object.item_brand_name+' ('+item_object.code+')</p>';
                }else{
                    item_to_show += '<p class="item_name">'+item_object.item_name+' ('+item_object.code+')</p>';
                }
                item_to_show += '</div>';

                if($('#category_'+item_object.category_id).length>0){

                    $('#category_'+item_object.category_id).append(item_to_show);
                }else{
                    var category_to_show = '<div id="category_'+item_object.category_id+'"  class="category_'+item_object.category_id+' specific_category_items_holder" style="display: block;">';
                    category_to_show += item_to_show;
                    category_to_show += '</div>';
                    $('.category_items').append(category_to_show);
                }
                $('#searched_item_found').append(item_to_show);

                $option_item = '<option value="'+shaped_item_object.item_id+'">'+shaped_item_object.item_name+'</option>';
                $('#all_items_options').append($option_item);

                $('#add_item_modal').slideUp('500');
                $("#item_"+shaped_item_object.item_id).click();

                $("#modal_item_type").html(item_type);
                var item_sale_unit_modal_tmp_1 = $("#item_sale_unit_modal").val();

                var item_opening_stock_modal_tmp_1 = 0;
                var item_sale_unit_modal_tmp_2 = '';
                if(item_opening_stock_modal){
                    item_opening_stock_modal_tmp_1 = item_opening_stock_modal;
                }
                if(item_sale_unit_modal_tmp_1){
                    item_sale_unit_modal_tmp_2 = $("#item_sale_unit_modal option:selected").text();
                }
                $(".current_stock_t").html(item_opening_stock_modal_tmp_1+" "+item_sale_unit_modal_tmp_2);
                ...
                $('#item_sale_unit_modal').val('').change();
                $('#item_sale_price_modal').val('');
                $('#item_whole_sale_price_modal').val('');
                $('#item_purchase_unit_modal').val('').change();
                $('#item_conversion_rate_modal').val('');
                $('#item_purchase_price_modal').val('');
                $('#item_default_qty_amt_modal').val('');
    ...
    $(document).on('click','.remove_this_item_from_cart',function(){
        var row_number = $(this).attr('data-remove-order-row-no');
        $('.single_order[data-single-order-row-no='+row_number+']').remove();
        do_addition_of_item_and_modifiers_price();
    });

    // $('#item_quantity_modal_input').numpad();
    // $('#modal_discount').numpad();
    // $('#sub_total_discount').numpad();
    // $('#delivery_charge').numpad();
    // $('.nmpd-grid').css('background-color','#cfcfcf');
    ...
    $(".add_item_modal_select").select2({ dropdownAutoWidth : true });
    $("#all_items_options").select2({dropdownAutoWidth:true});

    /**
     * Icon Pick Date
    */
    $('.icon_pick_date').click(function() {
        $('.search_sale').show().focus().hide();
    });
    /**
     * Icon Pick Date
    */
    $('.icon_pick_note').on('click', function() {
        Swal.fire({
            title: 'Type a note',
            input: 'text',
            inputAttributes: {
            ...
            // $('#sale_remark1').show();
            $('#sale_remark1').val(result.value);
        })
        
        $('.swal2-input' ).val($('#sale_remark1').val());
    });
});
...
function update_all_total_price(){
    ...
    //get item quantity
    var item_quantity = (parseFloat($('#item_quantity_modal').html())>parseFloat(0))?parseFloat($('#item_quantity_modal').html()).toFixed(2):parseFloat(0).toFixed(2);
    //get item unit price
    var item_unit_price = parseFloat($('#modal_item_price_input_field').val()).toFixed(2);
    ...
    var all_price = parseFloat(item_price_after_discount).toFixed(2);
...
function do_addition_of_item_and_modifiers_price(){
    ...
    var total_items_in_cart_with_quantity = 0;
    $('.order_holder .single_order .first_portion').each(function(i, obj) {
        total_items_in_cart_with_quantity = parseInt(total_items_in_cart_with_quantity)+parseInt($(this).find('.third_column span').eq(0).html());
    });
    ...
    // $('#total_items_in_cart_with_quantity').html(total_of_all_items_and_modifiers);
...
function get_total_vat(){
    ...
    $('.single_order .first_portion .fifth_column span').each(function(i, obj) {
       ...
       // var item_total_price = parseFloat($(this).parent().parent().find('.fifth_column span').html()).toFixed(2);
        var item_total_price = parseFloat($(this).parent().parent().find('.fifth_column span').attr('data-total_amount')).toFixed(2);
...
function reset_on_modal_close_or_add_customer(){
    $('#add_customer_modal').slideUp('500');
    ...
    $('#customer_previous_due_modal').val('');
}
...
function add_sale_by_ajax(order_object,sale_id,total_payable){

    var finalize_previous_due = $("#finalize_previous_due").html();
    $('#finalize_grand_total').html(total_payable);
    $('#finalize_total_payable').html((Number(total_payable)+(Number(finalize_previous_due))).toFixed(2));

    $('#pay_amount_invoice_input').val((Number(total_payable)+(Number(finalize_previous_due))).toFixed(2));
...
function print_invoice_and_close(sale_id,payment_method_type,invoice_create_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due,account_note){

    if(invoice_create_type == 1){ //if type is 1 then update order status to invoiced order, and update payment method type
       // update_order_status_to_invoiced(sale_id,payment_method_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email);
       update_order_status_to_invoiced(sale_id,payment_method_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,account_note); //added by alauddin
    }else if(invoice_create_type == 2){ //then change order status to close, close time update, payment method type update,
       // close_order(sale_id,payment_method_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due);
       close_order(sale_id,payment_method_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due,account_note); //added by alauddin
    }
    print_invoice(sale_id);
}
    ...
    $.ajax({
        ...
        success:function(response) {
            ...
            for (var key in orders) {
                ...
                held_orders += '<div class="third_column column fix">'+orders[key].sale_time+'</div>';
                held_orders += '</div>';
            }
...
function update_order_status_to_invoiced(sale_id,payment_method_type,paid_amount,due_amount,send_invoice_sms,send_invoice_email,account_note){
    $.ajax({
        ...
        success:function(response) {
            if(response>1){
                $("#cid_"+response.customer_id).attr("data-previous_due",response.customer_new_due);
                set_new_orders_to_view();
            }
        },
        error:function(){
...
function close_order(sale_id,payment_method_type,paid_amount, due_amount,send_invoice_sms,send_invoice_email,finalize_previous_due,account_note){
    $.ajax({
        url:base_url+"Sale/update_order_status_ajax",
        method:"POST",
        dataType:"json",
...
function print_invoice(sale_id){
   ...
   var newWindow='';
    if (print_format == "Thermal Print") {
        var newWindow = open("print_invoice/"+sale_id, 'Print Invoice', 'width=450,height=550')
    } else if (print_format == "A4 Print") {
        var newWindow = open("print_invoice/"+sale_id, 'Print Invoice', 'width=1600,height=550')
    } else if (print_format == "Half A4 Print") {
        var newWindow = open("print_invoice/"+sale_id, 'Print Invoice', 'width=421,height=595')
    }
    ...
    $.ajax({
        ...
        success:function(response) {
            ...
            for (var key in response.items) {
                ...
                draw_table_for_order += '<div class="single_order fix"  data-qty_default="'+this_item.default_qty_amt+'" id="order_for_item_'+this_item.food_menu_id+'">';
                draw_table_for_order += '<div class="first_portion fix">';
                ...
                draw_table_for_order += '<div class="single_order_column first_column fix"><i style="cursor:pointer;" data-status="edit"  class="fas fa-pencil-alt edit_item_first_call edit_item_increment edit_item_increment_'+this_item.food_menu_id+' edit_item" id="edit_item_'+this_item.food_menu_id+'"></i> <span  class="edit_item_ edit_item_new_'+this_item.food_menu_id+'" data-id="'+this_item.food_menu_id+'"  id="item_name_table_'+this_item.food_menu_id+'">'+this_item.menu_name+'</span></div>';
                ...
                draw_table_for_order += '<div class="single_order_column third_column fix"> <i style="cursor:pointer;" class="fas fa-minus-circle decrease_item_qty dd"  data-item_id="'+this_item.food_menu_id+'"></i> <span id="item_quantity_table_'+this_item.food_menu_id+'">'+parseFloat((this_item.qty)).toFixed(2)+'</span> <i style="cursor:pointer;" class="fas fa-plus-circle increase_item_qty" data-item_id="'+this_item.food_menu_id+'"></i>  <span class="cart_item_unit_name_'+this_item.food_menu_id+' cart_item_unit_name">'+this_unit_name+'</span></div>';
                draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox access_control" id="percentage_table_'+this_item.food_menu_id+'" value="'+this_item.menu_discount_value+'"></div>';
                draw_table_for_order += '<div class="single_order_column fifth_column fix">'+currency+' <span data-total_amount="" id="item_total_price_table_'+this_item.food_menu_id+'">'+this_item.menu_price_with_discount+'</span><i data-remove-order-row-no="0" class="remove_this_item_from_cart fas fa-times-circle"></i></div>';
                draw_table_for_order += '</div>';
                if(this_item.menu_note!=""){
                    draw_table_for_order += '<div class="third_portion fix">';
                    draw_table_for_order += '<div class="single_order_column first_column fix">IMEI/Serial: <span id="item_note_table_'+this_item.food_menu_id+'">'+this_item.menu_note+'</span></div>';
                    draw_table_for_order += '</div>';
                }
    ...
    $.ajax({
        ...
        success:function(response) {
            ...
            for (var key in response.items) {
                ...
                if(this_item.menu_note!=""){
                    draw_table_for_order_details += '<div class="third_portion fix">';
                    draw_table_for_order_details += '<div class="single_order_column_hold first_column column fix">IMEI/Serial: <span id="order_details_item_note_table_'+this_item.food_menu_id+'">'+this_item.menu_note+'</span></div>';
                    draw_table_for_order_details += '</div>';
                }
...
function reset_last_10_sales_modal(){
    ...
    $("#paid_amount_last_10").html('0.00');
    $("#due_amount_last_10").html('0.00');
...
function reset_finalize_modal(){
    ...
    var selected_id = ($("#finalie_order_payment_method option:contains(Cash)").val());
...
function arrange_info_on_the_cart_to_modify(response){
    ...
    for (var key in response.items) {
        ...
        draw_table_for_order += '<div class="single_order_column second_column fix">'+currency+' <span id="item_price_table_'+this_item.food_menu_id+'">'+this_item.menu_unit_price+'</span></div>';
        draw_table_for_order += '<div class="single_order_column third_column fix"><i style="cursor:pointer;" class="fas fa-minus-circle decrease_item_table" id="decrease_item_table_'+this_item.food_menu_id+'"></i> <span id="item_quantity_table_'+this_item.food_menu_id+'">'+(this_item.qty).toFixed(2)+'</span> <i style="cursor:pointer;" class="fas fa-plus-circle increase_item_table" id="increase_item_table_'+this_item.food_menu_id+'"></i></div>';
        draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox access_control" id="percentage_table_'+this_item.food_menu_id+'" value="'+this_item.menu_discount_value+'" disabled></div>';
...
function get_customer_for_edit(customer_id){
    $.ajax({
        ...
        success:function(response) {
            ...
            $('#customer_previous_due_modal').val(response.previous_due);
...
function find_item_by_item_id(item_id){
    for(i in window.items){
        var single_item = window.items[i];
...
function find_item_by_item_code(item_code){
    for(i in window.items){
        var single_item = window.items[i];
        if(single_item.item_code == item_code){
            return single_item;
        }
    }
    return null;
}
function ready_item_modal(item_id){
    //set item id to modal
    $('#modal_item_id').html(item_id);
    ...
    if(item_object.item_type==2){
        ...
        unit_name = item_object.sale_unit_name
    }

    $('#sale_unit_name_modal').html(unit_name);
    //get item/menu name from right item
    ...
    //get item/menu price from right side
    var item_price = (parseFloat(item_object.price.substr(2))).toFixed(2);
    //set item/menu price to modal
    $('#modal_item_price').html(item_price);
    $('#s_price').html(item_price);
    $('#modal_item_price_input_field').val(item_price);
    ...
    var default_qty_amt = (item_object.default_qty_amt=="" || item_object.default_qty_amt==null || item_object.default_qty_amt==0)?1: item_object.default_qty_amt;
    $('#item_quantity_modal_input').val(default_qty_amt);
    $('#item_quantity_modal').html(default_qty_amt);
...
$('#modal_discount1,#pay_amount_invoice_input1,#given_amount_input1,#sub_total_discount1,#delivery_charge1')
    .keyboard({
...
// function is_inside_or_outside_of_modal(id,event){

// 	if(modal_id.includes(e.target.id) || $(event.target).parents("#"+e.target.id).length){
// }
...
$('body').on('click','.remove_this_item_from_cart',function(){
    productSound3.play();
});
