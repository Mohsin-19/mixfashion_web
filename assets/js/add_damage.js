...
$(function () {
    ...
    $('#food_menu_damage_button').on('click',function(){
        ...
        if(quantity==""){
            ...
            $("#food_menu_damage_quantity_err_msg").text("The Quantity field is required.");
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, v) {
                        ...
                        options += '<tr class="rowCount" data-id="' + i + '" id="row_' + i + '">'+
                            ...
                            '<input type="hidden" id="product_id_' + i + '" name="product_id[]" value="' + v.product_id + '"/>'+
                            '<td><span style="padding-bottom: 5px;">'+v.name+"("+v.code+")"+'</span></td>'+
                            '<td style="width: 15%"><input readonly  type="text" data-countID="'+i+'" id="damage_amount_' + i + '" name="damage_amount[]" onfocus="this.select();" class="form-control integerchk aligning" placeholder="Damage Qty/Amt" value=" '+qty+' " /><span class="label_aligning"> ' +v.unit_name+ '</span><span id="unit_consumption_product_'+i+'" class="unit_consumption_product" style="display:none">'+v.consumption+'</span></td>'+
                            '<input type="hidden" id="last_purchase_price_' + i + '" name="last_purchase_price[]" value="' + i + '"/>'+
                            '<td><input type="text" autocomplete="off" id="loss_amount_' + i + '" name="loss_amount[]" onfocus="this.select();" class="form-control aligning" placeholder="Loss Amt" value=" '+los_amount+' " /><span class="label_aligning">'+currency+'</span><span id="unit_price_product_'+i+'" class="unit_price_product" style="display:none">'+v.unit_price+'</span></td>'+
    ...
    $('#product_id').on('change',function(){
        ...
        if (product_details != '') {
            ...
            var cart_row = '<tr class="rowCount" data-id="' + suffix + '" id="row_' + suffix + '">'+
                ...
                '<td style="width: 15%"><input type="text" autocomplete="off" data-countID="'+suffix+'" id="damage_amount_' + suffix + '" name="damage_amount[]" onfocus="this.select();" class="form-control integerchk aligning" placeholder="'+damage_amt+'" /><span class="label_aligning"> ' + product_details_array[2] + '</span></td>'+
            '<input type="hidden" id="last_purchase_price_' + suffix + '" name="last_purchase_price[]" value="' + product_details_array[3] + '"/>'+
            '<td><input type="text" autocomplete="off" id="loss_amount_' + suffix + '" name="loss_amount[]" onfocus="this.select();" class="form-control aligning" onkeyup="return calculateAll();" placeholder="'+loss_amt+'"  /><span class="label_aligning">'+currency+'</span></td>'+
            '<td><a class="btn btn-danger btn-xs" style="margin-left: 5px; margin-top: 10px;" onclick="return deleter(' + suffix + ',' + product_details_array[0] +');" ><i class="fa fa-trash"></i> </a></td>'+
            '</tr>';
    ...
    $('#damage_form').on('submit',function(){
        ...
        $(".integerchk").each(function () {
            ...
            if(damage_amount == '' || isNaN(damage_amount)){
                $("#damage_amount_"+n).css({"border-color":"red"}).show(200).delay(2000,function(){
...
function calculateAll(){
    ...
    $(".rowCount").each(function () {
        ...
        if (typeof(damage_amount) !== "undefined" && damage_amount !== null) {
            ...
            var loss_amount = parseFloat($.trim(damage_amount)) * parseFloat($.trim(last_purchase_price));
...
function updateRowNo(){
    ...
    for(var r=0;r<numRows;r++){
        $("#damage_cart tbody tr").eq(r).find("td:first p").text(r+1);
    }
}
